<center>
<div class="contenedor-menu" style="margin-bottom:5%; justify-content: middle;">

<button onclick="usuarios()" class="botonmenu2" type="Submit"> <b class="letter">Usuarios</b> <img class="registro_login" src="<?php echo base_url('assets/img/usuario.png');?>"></button> 

<button onclick="colegios()" class="botonmenu2" type="Submit"> <b class="letter">Colegios</b> <img class="registro_colegios" src="<?php echo base_url('assets/img/Colegios.png');?>"></button> 

<a href="<?php echo base_url('index.php/CReportes/acerca_de') ?>">
    <button  class="botonmenu2" href="" id=""> <b class="letter">Acerca de</b> <img style="height: 112px;" src="<?php echo base_url('assets/img/acerca-de.png');?>"></button> 
</a>

<button  class="botonmenu2" href="" id="cerrarSession"> <b class="letter">Cerrar Sesión</b> <img style="height: 112px;" src="<?php echo base_url('assets/img/salir.png');?>"></button> 
</div>
</center>